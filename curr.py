...
def CheckTrainingDoneCallback(reward_array, done_array, env):
	...
	if len(done_array) > 30:
		...
		if done_cond == True:
			if env < 3:
				if np.mean(reward_array[-10:]) > 950: # Reward threshold on the final environment is higher because no reward shaping
					reward_cond = True
			else:
				if np.mean(reward_array[-10:]) > 800: # Reward threshold on the final environment is lower because no reward shaping
					reward_cond = True
...
def parameterizing_function(navigation, breaking, crafting, prev_width, prev_height):
	...
	if type_of_env == 0:
		object_present = np.random.randint(0,3) # 0 -> cube1, 1 -> cube2, 2 -> Target location
		if object_present == 0:
			no_trees = 1
			no_rocks = 0
			crafting_table = 0
	...
	if type_of_env == 1:
		...
		object_present = np.random.randint(0,2)

		if object_present == 0:
			no_trees = 1
			no_rocks = 0
			crafting_table = 0
		elif object_present == 1:
			no_trees = 0
			no_rocks = 1
			crafting_table = 0
	...
	return width, height, no_trees, no_rocks, crafting_table, type_of_env
...
if __name__ == "__main__":
	...
	curriculum_params = [[{'width0': 0, 'height0': 0, 'no_trees': 0, 'no_rocks': 0, 'crafting_table': 0, 'navigation': 0, 'breaking': 0, 'crafting': 0, 'type_of_env0': 0}] for _ in range(beam_search_width)]

	actionCnt = 5
	D = 33 #8 beams x 4 items lidar + 5 inventory items
	NUM_HIDDEN = 10
	GAMMA = 0.995
	LEARNING_RATE = 1e-3
	DECAY_RATE = 0.99
	...
	for i in range(no_of_environmets):
		...
		width, height, no_trees, no_rocks, crafting_table, type_of_env = parameterizing_function(navigation = 0, breaking = 0, crafting = 0, prev_width = 0, prev_height = 0)
		...
		env = gym.make(env_id, map_width = width, map_height = height, items_quantity = {'cube1': no_trees, 'cube2': no_rocks, 'crafting_table': crafting_table}, goal_env = type_of_env, is_final = final_status)
	...
	curriculum_params_1 = [[{'width0': 0, 'height0': 0, 'no_trees': 0, 'no_rocks': 0, 'crafting_table': 0, 'navigation': 0, 'breaking': 0, 'crafting': 0, 'type_of_env0' : 0,\
	'width1': 0, 'height1': 0, 'no_trees1': 0, 'no_rocks1': 0, 'crafting_table1': 0, 'type_of_env1' : 0}] for _ in range(beam_search_width)]

	curriculum_params_2 = [[{'width0': 0, 'height0': 0, 'no_trees': 0, 'no_rocks': 0, 'crafting_table': 0, 'navigation': 0, 'breaking': 0, 'crafting': 0, 'type_of_env0' : 0, \
	'width1': 0, 'height1': 0, 'no_trees1': 0, 'no_rocks1': 0, 'crafting_table1': 0, 'type_of_env1' : 0, \
	'width2': 0, 'height2': 0, 'no_trees2': 0, 'no_rocks2': 0, 'crafting_table2': 0, 'type_of_env2' : 0}] for _ in range(beam_search_width)]
	...
	for curriculum_number in range(1, curriculum_breadth):
		...
		for beam_number in range(beam_search_width):
			for environment_number in range(no_of_environmets):
				...
				width, height, no_trees, no_rocks, crafting_table, type_of_env = parameterizing_function(navigation = no_navigation, breaking = no_breaking, crafting = no_crafting, prev_width = prev_width, prev_height = prev_height)
	...
	for final_env in range(beam_search_width):
		...
		env = gym.make(env_id, map_width = 10, map_height = 10, items_quantity = {'cube1': 1, 'cube2': 1, 'crafting_table': 1}, goal_env = 2, is_final = final_status)
		...
		while True:
			...
			if t_step > t_limit or done == True:
				...
				# quit after some number of episodes
				if episode > 800000 or env_flag == 1:
